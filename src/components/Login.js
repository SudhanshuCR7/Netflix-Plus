...
const Login = () => {
  ...
  const handleValidation = () => {
    const message = validateFormFiedls(email.current.value,password.current.value)
  ...
  return (
    ...
    <form onSubmit={(e) => e.preventDefault()} className='absolute bg-black my-36 mx-auto right-0 left-0 w-3/12  text-white p-12 rounded-lg' >
      <h1 className='font-bold text-3xl py-2'>
        {isSignInForm ? 'Sign In' : 'Sign Up'}
      </h1>
      {!isSignInForm && (<input type='text' placeholder='Full Name' className='p-3 my-4 w-full rounded-lg bg-gray-700'></input>)}
      <input 
       ref={email}
       type='text' 
       placeholder='Email' 
      ...
      <button className='p-3 my-6 mt-8 w-full bg-red-700 rounded-lg' onClick={handleValidation}>
         {isSignInForm ? 'Sign In' : 'Sign Up'}
      </button> 
      <p 
